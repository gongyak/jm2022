...
const selectTag = (tag) => {
    ...
    gtag('event', 'jm2022:카테고리 선택', {
        'event_category': '카테고리',
        'event_lavel': `${tag}`
    })
    gtag('event', `jm2022:카테고리: ${tag}`, {
        'event_category': '카테고리',
        'event_lavel': `${tag}`
    })
}
...
const expandPledge = (index) => {
    ...
    if (element_title || element_location) {
        ...
        gtag('event', 'jm2022:세부공약 선택', {
            'event_category': '세부공약',
            'event_lavel': `${title} ${location}`
        })
        gtag('event', `jm2022:세부공약: ${title} ${location}`, {
            'event_category': '세부공약',
            'event_lavel': `${title} ${location}`
        })
...
const selectSection = (section) => {
    ...
    switch (section) {
        case 0:
            ...
            gtag('event', 'jm2022:섹션전환', {
                'event_category': '섹션',
                'event_lavel': `관심분야`
            })
            gtag('event', 'jm2022:섹션전환: 관심분야', {
                'event_category': '섹션',
                'event_lavel': `관심분야`
            })
